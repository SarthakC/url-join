...
describe("url join", () => {
  ...
  it("should support file protocol urls with windows paths", () => {
    expect(urlJoin("file:/", "C:/", "foo/bar")).toEqual("file://C:/foo/bar");
    expect(urlJoin("file:", "C:/", "foo/bar")).toEqual("file://C:/foo/bar");
  });
  ...
  it("should fail with segments that are not string", () => {
    ...
    expect(() => urlJoin(true)).toThrowError(
      /Url must be a string. Received true/
    );
    // @ts-expect-error testing with invalid input
    expect(() => urlJoin("http://blabla.com/", 1)).toThrowError(
      /Url must be a string. Received 1/
    );
    // @ts-expect-error testing with invalid input
    expect(() => urlJoin("http://blabla.com/", undefined, "test")).toThrowError(
      /Url must be a string. Received undefined/
    );
    // @ts-expect-error testing with invalid input
    expect(() => urlJoin("http://blabla.com/", null, "test")).toThrowError(
      /Url must be a string. Received null/
    );
    expect(() =>
      // @ts-expect-error testing with invalid input
      urlJoin("http://blabla.com/", { foo: 123 }, "test")
    ).toThrowError(/Url must be a string. Received \[object Object\]/);
  });
